...
class Scraper:
    ...
    def __init__(self, wiki_url, phrase, use_local_html_file_instead=False):
        ...
        :param wiki_url: URL link to the wiki which will be scraped from
        :type wiki_url: any

        :param phrase: Title of the wiki article page which will be scraped
        :type phrase: str

        :param use_local_html_file_instead: True/False if wiki_url is a path to a single local HTML file.
        ...
        self.soup = None  # Page content will be loaded here as a BeautifulSoup instance
    ...
    def fetch_data(self):
        """
        Fetches data from either a local file or a remote URL depending on the `read_local_file` attribute. If
        `read_local_file` is set to True, it tries to fetch data from a local file using
        the `fetch_data_from_local_file` method. Otherwise, data is fetched from a remote
        URL using the `fetch_data_from_wiki` method. If there was an error, ConnectionError is raised.

        :return: `True` if data is successfully fetched and the soup object is initialized.
        :rtype: bool
        :raises ConnectionError: If data loading from local file or remote URL fails,
            or if the soup object cannot be initialized.
        """
        if self.read_local_file:
            if not self.fetch_data_from_local_file():
                raise ConnectionError(f"Failed to load data from local file: {self.base_url}")
        else:
            if not self.fetch_data_from_wiki():
                raise ConnectionError(f"Failed to load data from {self.exact_url}")
        if not self.soup:
    ...
    def get_summary(self):
        """
        Gets the first paragraph of an article corresponding with Scraper's phrase.
        :return: String value of the first paragraph's text or None if nothing was found.
        """
        if not self.soup:
         self.fetch_data()
        ...
        for p in paragraphs:
            ...
            if text:
                return text  # read text from paragraphs until the first non-null appears
        return None
    ...
    def get_table(self, table_number, first_row_header=False):
        ...
        into a pandas DataFrame. Supports optionally treating the first row of the table as the
        header for the resulting DataFrame.

        :param table_number: Specifies the table index to extract (1-based index).
        :type table_number: int

        :param first_row_header: Indicates whether the first row of the table should be used as the
            DataFrame's header. If True, the first row is used as column names; otherwise, no column
            headers are assigned. Default is False.
        :type first_row_header: bool

        :return: A pandas DataFrame object containing the desired table's data if successfully
            extracted, or None if the table is unavailable or an error occurs during the extraction.
        ...
        if len(tables) < table_number:
            print(f"Asked for {table_number} table, but only {len(tables)} are available.")
            return None
    ...
    def count_words(self):
        ...
        # Count words from both the main article content and title of the article
        title_soup = self.soup.find("h1", class_="firstHeading mw-first-heading")
        ...
        words = []  # List of all processed words of the current article that will be counted
        for word in raw_words:
            # Remove punctuation and irrelevant characters from the edges.
            # Words having non-letter characters inside (without any blank space around non-letter character) are
            # treated as a single word (e.g. "II/IV" is considered one word: "II/IV", "word." is processed into "word")
